...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = chars.distinct.map(f => (f, chars.count(_ == f)))
   ...
   */
    ...
    freqs.sortWith((t1, t2) => t1._2 < t2._2).map{ case (char, weight) => Leaf(char, weight) }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse(codeTree: CodeTree, bits: List[Bit]): List[Char] = codeTree match {
        case Leaf(char, _) if bits.isEmpty => List(char)
        case Leaf(char, weight) => char :: traverse(tree, bits)
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def look(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List.empty
        case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: look(left)(c)
        case Fork(left, right, _, _) => 1 :: look(right)(c)
      }

      text.flatMap(look(tree))
    }
